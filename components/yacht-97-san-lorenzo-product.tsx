...
import { SAN_LORENZO_97_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { SAN_LORENZO_97_GALLERY_PATHS } from "@/lib/san-lorenzo-97-gallery";
import {
  SAN_LORENZO_97_DURATIONS,
  SAN_LORENZO_97_GRATUITY_PCTS,
  computeSanLorenzo97Totals,
  formatUsd,
  type SanLorenzo97GratuityPct,
} from "@/lib/san-lorenzo-97-pricing";
...
export function Yacht97SanLorenzoProduct() {
  const paths = SAN_LORENZO_97_GALLERY_PATHS;
  ...
  const [durationId, setDurationId] =
    useState<(typeof SAN_LORENZO_97_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] = useState<SanLorenzo97GratuityPct>(18);

  const tier = useMemo(
    () => SAN_LORENZO_97_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computeSanLorenzo97Totals(tier.baseUsd, gratuityPct),
    ...
    <>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
                ...
                <>
                  <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                    <Image
                      ...
                      alt={`97 San Lorenzo — photo ${photoIndex + 1} of ${galleryLen}`}
                      fill
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                <p
                  ...
                  id="san-lorenzo-97-duration-label"
                >
                  Duration
                </p>
                <div
                  ...
                  role="group"
                  aria-labelledby="san-lorenzo-97-duration-label"
                >
                  {SAN_LORENZO_97_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                <p
                  className="text-[0.58rem] font-medium uppercase tracking-[0.26em] text-cream/40"
                  id="san-lorenzo-97-gratuity-label"
                >
                  Crew gratuity
                </p>
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="san-lorenzo-97-gratuity-label"
                >
                  {SAN_LORENZO_97_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
          ...
          <YachtListingAbout {...SAN_LORENZO_97_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
              { k: "Overall length", v: "97′ feet" },
              { k: "Capacity", v: "13 cruising" },
              { k: "Staterooms", v: "4" },
              { k: "Bathrooms", v: "5" },
            ]}
            {...SAN_LORENZO_97_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
