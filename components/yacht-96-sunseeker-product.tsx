...
import { SUNSEEKER_96_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { SUNSEEKER_96_GALLERY_PATHS } from "@/lib/sunseeker-96-gallery";
import {
  SUNSEEKER_96_DURATIONS,
  SUNSEEKER_96_GRATUITY_PCTS,
  computeSunseeker96Totals,
  formatUsd,
  type Sunseeker96GratuityPct,
} from "@/lib/sunseeker-96-pricing";
...
export function Yacht96SunseekerProduct() {
  const paths = SUNSEEKER_96_GALLERY_PATHS;
  ...
  const [durationId, setDurationId] =
    useState<(typeof SUNSEEKER_96_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] = useState<Sunseeker96GratuityPct>(18);

  const tier = useMemo(
    () => SUNSEEKER_96_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computeSunseeker96Totals(tier.baseUsd, gratuityPct),
    ...
    <>
      <SectionReveal className="relative overflow-hidden border-b border-gold/10 bg-background">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-10 pt-14 sm:px-6 sm:pb-12 sm:pt-20 lg:px-10">
          ...
          <p className="mt-4 font-serif text-2xl tabular-nums text-gold-secondary sm:text-[1.75rem]">
            from {formatUsd(6_900)}
          </p>
          <p className="mt-5 max-w-2xl text-sm leading-relaxed text-cream/58 sm:text-[0.9375rem]">
            Miami Beach Marina — floating pool, two Sunchill pads, and dinghy
            included. Charter, tax, and gratuity below.
          </p>
        </div>
      </SectionReveal>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
                ...
                <>
                  <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                    <Image
                      ...
                      alt={`96 Sunseeker — photo ${photoIndex + 1} of ${galleryLen}`}
                      fill
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                <p
                  ...
                  id="sunseeker-96-duration-label"
                >
                  Duration
                </p>
                <div
                  ...
                  role="group"
                  aria-labelledby="sunseeker-96-duration-label"
                >
                  {SUNSEEKER_96_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                <p
                  className="text-[0.58rem] font-medium uppercase tracking-[0.26em] text-cream/40"
                  id="sunseeker-96-gratuity-label"
                >
                  Crew gratuity
                </p>
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="sunseeker-96-gratuity-label"
                >
                  {SUNSEEKER_96_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
          ...
          <YachtListingAbout {...SUNSEEKER_96_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
              { k: "Overall length", v: "96′ feet" },
              { k: "Capacity", v: "12 cruising" },
              { k: "Staterooms", v: "4" },
              { k: "Bathrooms", v: "5" },
            ]}
            {...SUNSEEKER_96_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
