...
import { PERSHING_90_REFIT_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { PERSHING_90_REFIT_GALLERY_PATHS } from "@/lib/pershing-90-refit-gallery";
import {
  PERSHING_90_REFIT_DURATIONS,
  PERSHING_90_REFIT_GRATUITY_PCTS,
  computePershing90RefitTotals,
  formatUsd,
  type Pershing90RefitGratuityPct,
} from "@/lib/pershing-90-refit-pricing";
...
export function Yacht90PershingRefitProduct() {
  const paths = PERSHING_90_REFIT_GALLERY_PATHS;
  ...
  const [durationId, setDurationId] =
    useState<(typeof PERSHING_90_REFIT_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] =
    useState<Pershing90RefitGratuityPct>(18);

  const tier = useMemo(
    () => PERSHING_90_REFIT_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computePershing90RefitTotals(tier.baseUsd, gratuityPct),
    ...
    <>
      <SectionReveal className="relative overflow-hidden border-b border-gold/10 bg-background">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-10 pt-14 sm:px-6 sm:pb-12 sm:pt-20 lg:px-10">
          ...
          <p className="mt-4 font-serif text-2xl tabular-nums text-gold-secondary sm:text-[1.75rem]">
            from {formatUsd(6_000)}
          </p>
          <p className="mt-5 max-w-2xl text-sm leading-relaxed text-cream/58 sm:text-[0.9375rem]">
            2025 refit — Regal Marina or optional Miami Beach upgrade. Seabob &
            trampoline included; water slide available as an add-on. Rates below.
          </p>
        </div>
      </SectionReveal>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
                ...
                <>
                  <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                    <Image
                      ...
                      alt={`90 Pershing Refit 2025 — photo ${photoIndex + 1} of ${galleryLen}`}
                      fill
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                <p
                  ...
                  id="pershing-90-refit-duration-label"
                >
                  Duration
                </p>
                <div
                  ...
                  role="group"
                  aria-labelledby="pershing-90-refit-duration-label"
                >
                  {PERSHING_90_REFIT_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                <p
                  className="text-[0.58rem] font-medium uppercase tracking-[0.26em] text-cream/40"
                  id="pershing-90-refit-gratuity-label"
                >
                  Crew gratuity
                </p>
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="pershing-90-refit-gratuity-label"
                >
                  {PERSHING_90_REFIT_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
              ...
              <div className="mt-6">
                <YachtBookingButton
                  yachtId="90-pershing-refit-2025"
          ...
          <YachtListingAbout {...PERSHING_90_REFIT_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
              { k: "Overall length", v: "90′ feet" },
              { k: "Capacity", v: "13 cruising" },
              { k: "Staterooms", v: "4" },
              { k: "Bathrooms", v: "4" },
            ]}
            {...PERSHING_90_REFIT_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
