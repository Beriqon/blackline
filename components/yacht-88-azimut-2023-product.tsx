...
import { AZIMUT_88_2023_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { AZIMUT_88_GALLERY_PATHS } from "@/lib/azimut-88-gallery";
import {
  AZIMUT_88_DURATIONS,
  AZIMUT_88_GRATUITY_PCTS,
  computeAzimut88Totals,
  formatUsd,
  type Azimut88GratuityPct,
} from "@/lib/azimut-88-pricing";
...
export function Yacht88Azimut2023Product() {
  const galleryLen = AZIMUT_88_GALLERY_PATHS.length;
  ...
  const [durationId, setDurationId] =
    useState<(typeof AZIMUT_88_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] = useState<Azimut88GratuityPct>(18);

  const tier = useMemo(
    () => AZIMUT_88_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computeAzimut88Totals(tier.baseUsd, gratuityPct),
    ...
    <>
      <SectionReveal className="relative overflow-hidden border-b border-gold/10 bg-background">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-10 pt-14 sm:px-6 sm:pb-12 sm:pt-20 lg:px-10">
          ...
          <p className="mt-4 font-serif text-2xl tabular-nums text-gold-secondary sm:text-[1.75rem]">
            from {formatUsd(9_000)}
          </p>
          <p className="mt-5 max-w-2xl text-sm leading-relaxed text-cream/58 sm:text-[0.9375rem]">
            Island Gardens departure — Jet Ski, Seabob, floating mat, and snorkel
            gear included. See charter, tax, and gratuity below.
          </p>
        </div>
      </SectionReveal>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-14">
            <div className="min-w-0">
              <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                <Image
                  src={AZIMUT_88_GALLERY_PATHS[photoIndex]}
                  alt={`88 Azimut 2023 — photo ${photoIndex + 1} of ${galleryLen}`}
                  fill
              ...
              >
                {AZIMUT_88_GALLERY_PATHS.map((src, i) => (
                  <button
                    key={src}
                    ref={(node) => {
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                ...
                <div
                  ...
                  role="group"
                  aria-labelledby="azimut-88-duration-label"
                >
                  {AZIMUT_88_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                ...
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="azimut-88-gratuity-label"
                >
                  {AZIMUT_88_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
          ...
          <YachtListingAbout {...AZIMUT_88_2023_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
            ...
            {...AZIMUT_88_2023_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
