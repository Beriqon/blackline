...
import { PRINCESS_88_PFO_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { PRINCESS_88_PFO_GALLERY_PATHS } from "@/lib/princess-88-pfo-gallery";
import {
  PRINCESS_88_PFO_DURATIONS,
  PRINCESS_88_PFO_GRATUITY_PCTS,
  computePrincess88PfoTotals,
  formatUsd,
  type Princess88PfoGratuityPct,
} from "@/lib/princess-88-pfo-pricing";
...
export function Yacht88PrincessPfoProduct() {
  const paths = PRINCESS_88_PFO_GALLERY_PATHS;
  ...
  const [durationId, setDurationId] =
    useState<(typeof PRINCESS_88_PFO_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] =
    useState<Princess88PfoGratuityPct>(18);

  const tier = useMemo(
    () => PRINCESS_88_PFO_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computePrincess88PfoTotals(tier.baseUsd, gratuityPct),
    ...
    <>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
                ...
                <>
                  <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                    <Image
                      ...
                      alt={`88 Princess PFO — photo ${photoIndex + 1} of ${galleryLen}`}
                      fill
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                <p
                  ...
                  id="princess-88-pfo-duration-label"
                >
                  Duration
                </p>
                <div
                  ...
                  role="group"
                  aria-labelledby="princess-88-pfo-duration-label"
                >
                  {PRINCESS_88_PFO_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                <p
                  className="text-[0.58rem] font-medium uppercase tracking-[0.26em] text-cream/40"
                  id="princess-88-pfo-gratuity-label"
                >
                  Crew gratuity
                </p>
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="princess-88-pfo-gratuity-label"
                >
                  {PRINCESS_88_PFO_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
          ...
          <YachtListingAbout {...PRINCESS_88_PFO_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
              { k: "Overall length", v: "88′ feet" },
              { k: "Capacity", v: "13 cruising" },
              { k: "Staterooms", v: "4" },
              { k: "Bathrooms", v: "5" },
            ]}
            {...PRINCESS_88_PFO_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
