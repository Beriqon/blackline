...
import { SIRENA_88_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { SIRENA_88_GALLERY_PATHS } from "@/lib/sirena-88-gallery";
import {
  SIRENA_88_DURATIONS,
  SIRENA_88_GRATUITY_PCTS,
  computeSirena88Totals,
  formatUsd,
  type Sirena88GratuityPct,
} from "@/lib/sirena-88-pricing";
...
export function Yacht88SirenaProduct() {
  const paths = SIRENA_88_GALLERY_PATHS;
  ...
  const [durationId, setDurationId] =
    useState<(typeof SIRENA_88_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] = useState<Sirena88GratuityPct>(18);

  const tier = useMemo(
    () => SIRENA_88_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computeSirena88Totals(tier.baseUsd, gratuityPct),
    ...
    <>
      <SectionReveal className="relative overflow-hidden border-b border-gold/10 bg-background">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-10 pt-14 sm:px-6 sm:pb-12 sm:pt-20 lg:px-10">
          ...
          <p className="mt-4 font-serif text-2xl tabular-nums text-gold-secondary sm:text-[1.75rem]">
            from {formatUsd(8_500)}
          </p>
          <p className="mt-5 max-w-2xl text-sm leading-relaxed text-cream/58 sm:text-[0.9375rem]">
            Santa Maria on Brickell — jacuzzi, plunge pool, flybridge bar & grill,
            and a full toy list. Charter math below.
          </p>
        </div>
      </SectionReveal>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
                ...
                <>
                  <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                    <Image
                      ...
                      alt={`88 Sirena — photo ${photoIndex + 1} of ${galleryLen}`}
                      fill
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                ...
                <div
                  ...
                  role="group"
                  aria-labelledby="sirena-88-duration-label"
                >
                  {SIRENA_88_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                ...
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="sirena-88-gratuity-label"
                >
                  {SIRENA_88_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
          ...
          <YachtListingAbout {...SIRENA_88_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
            ...
            {...SIRENA_88_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
