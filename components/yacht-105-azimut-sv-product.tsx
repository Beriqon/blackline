...
import { AZIMUT_105_SV_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { AZIMUT_105_SV_GALLERY_PATHS } from "@/lib/azimut-105-sv-gallery";
import {
  AZIMUT_105_SV_DURATIONS,
  AZIMUT_105_SV_GRATUITY_PCTS,
  computeAzimut105SvTotals,
  formatUsd,
  type Azimut105SvGratuityPct,
} from "@/lib/azimut-105-sv-pricing";
...
export function Yacht105AzimutSvProduct() {
  const paths = AZIMUT_105_SV_GALLERY_PATHS;
  ...
  const [durationId, setDurationId] =
    useState<(typeof AZIMUT_105_SV_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] = useState<Azimut105SvGratuityPct>(18);

  const tier = useMemo(
    () => AZIMUT_105_SV_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computeAzimut105SvTotals(tier.baseUsd, gratuityPct),
    ...
    <>
      <SectionReveal className="relative overflow-hidden border-b border-gold/10 bg-background">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-10 pt-14 sm:px-6 sm:pb-12 sm:pt-20 lg:px-10">
          ...
          <p className="mt-4 font-serif text-2xl tabular-nums text-gold-secondary sm:text-[1.75rem]">
            from {formatUsd(7_500)}
          </p>
          <p className="mt-5 max-w-2xl text-sm leading-relaxed text-cream/58 sm:text-[0.9375rem]">
            105′ Azimut “Sale Vita” — Bayshore departure. MTU power, five staterooms,
            floating lounge, Jet Skis & Seabobs. Rates below.
          </p>
        </div>
      </SectionReveal>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
                ...
                <>
                  <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                    <Image
                      ...
                      alt={`105 Azimut SV — photo ${photoIndex + 1} of ${galleryLen}`}
                      fill
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                <p
                  ...
                  id="azimut-105-sv-duration-label"
                >
                  Duration
                </p>
                <div
                  ...
                  role="group"
                  aria-labelledby="azimut-105-sv-duration-label"
                >
                  {AZIMUT_105_SV_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                <p
                  className="text-[0.58rem] font-medium uppercase tracking-[0.26em] text-cream/40"
                  id="azimut-105-sv-gratuity-label"
                >
                  Crew gratuity
                </p>
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="azimut-105-sv-gratuity-label"
                >
                  {AZIMUT_105_SV_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
          ...
          <YachtListingAbout {...AZIMUT_105_SV_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
              { k: "Overall length", v: "105′ feet" },
              { k: "Capacity", v: "13 cruising" },
              { k: "Staterooms", v: "5" },
              { k: "Bathrooms", v: "4" },
            ]}
            {...AZIMUT_105_SV_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
