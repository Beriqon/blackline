...
import { PERSHING_GTX_LISTING_CONTENT } from "@/lib/yacht-listing-narrative-content";
import { PERSHING_GTX_GALLERY_PATHS } from "@/lib/pershing-gtx-gallery";
import {
  PERSHING_GTX_DURATIONS,
  PERSHING_GTX_GRATUITY_PCTS,
  computePershingGtxTotals,
  formatUsd,
  type PershingGtxGratuityPct,
} from "@/lib/pershing-gtx-pricing";
...
export function YachtPershingGtxProduct() {
  const galleryLen = PERSHING_GTX_GALLERY_PATHS.length;
  ...
  const [durationId, setDurationId] =
    useState<(typeof PERSHING_GTX_DURATIONS)[number]["id"]>("4h");
  const [gratuityPct, setGratuityPct] = useState<PershingGtxGratuityPct>(18);

  const tier = useMemo(
    () => PERSHING_GTX_DURATIONS.find((d) => d.id === durationId)!,
    [durationId],
  );

  const { taxUsd, gratuityUsd, totalUsd } = useMemo(
    () => computePershingGtxTotals(tier.baseUsd, gratuityPct),
    ...
    <>
      <SectionReveal className="relative overflow-hidden border-b border-gold/10 bg-background">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-10 pt-14 sm:px-6 sm:pb-12 sm:pt-20 lg:px-10">
          ...
          <p className="mt-4 font-serif text-2xl tabular-nums text-gold-secondary sm:text-[1.75rem]">
            from {formatUsd(17_000)}
          </p>
          <p className="mt-5 max-w-2xl text-sm leading-relaxed text-cream/58 sm:text-[0.9375rem]">
            2022 Pershing GTX116 — Island Gardens or Miami Beach Marina. Sport
            utility layout with beach club and triple jets. See charter math below.
          </p>
        </div>
      </SectionReveal>
      ...
      <SectionReveal className="overflow-x-clip border-b border-gold/10 bg-charcoal pb-16 pt-10 sm:pb-20 sm:pt-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-14">
            <div className="min-w-0">
              <div className="relative aspect-[16/10] overflow-hidden border border-gold/12 bg-[#0b0b0b]">
                <Image
                  src={PERSHING_GTX_GALLERY_PATHS[photoIndex]}
                  alt={`116 Pershing GTX — photo ${photoIndex + 1} of ${galleryLen}`}
                  fill
              ...
              >
                {PERSHING_GTX_GALLERY_PATHS.map((src, i) => (
                  <button
                    key={src}
                    ref={(node) => {
            ...
            <div className="min-w-0">
              ...
              <div className="mt-8">
                <p
                  ...
                  id="pershing-gtx-duration-label"
                >
                  Duration
                </p>
                <div
                  ...
                  role="group"
                  aria-labelledby="pershing-gtx-duration-label"
                >
                  {PERSHING_GTX_DURATIONS.map((d) => (
                    <button
                      key={d.id}
                      type="button"
              ...
              <div className="mt-8">
                <p
                  className="text-[0.58rem] font-medium uppercase tracking-[0.26em] text-cream/40"
                  id="pershing-gtx-gratuity-label"
                >
                  Crew gratuity
                </p>
                <div
                  className="mt-2.5 flex flex-wrap gap-2"
                  role="group"
                  aria-labelledby="pershing-gtx-gratuity-label"
                >
                  {PERSHING_GTX_GRATUITY_PCTS.map((pct) => (
                    <button
                      key={pct}
                      type="button"
          ...
          <YachtListingAbout {...PERSHING_GTX_LISTING_CONTENT.about} />
          <YachtListingSpecsAndIncluded
            specRows={[
            ...
            {...PERSHING_GTX_LISTING_CONTENT.included}
          />
        </div>
      </SectionReveal>
